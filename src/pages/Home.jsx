    ...
    <HomeTemplete>
      <div>
        ...
        <h3 className="subtitle">
          모든 기능은 로그인을 해야 이용할 수 있으나, 다른 사람의 일정 보기에는
          로그인이 요구되지 않습니다.
        </h3>
      </div>
      <div className="button-region">
        <button value={registerText} className="homeBtn" onClick={onClickBtn}>
          1. 회원가입
        </button>
        <button value={loginText} className="homeBtn" onClick={onClickBtn}>
          2. 로그인
        </button>
        <button value={makeplanText} className="homeBtn" onClick={onClickBtn}>
          3. 일정만들기
        </button>
        <button value={mypageText} className="homeBtn" onClick={onClickBtn}>
          4. 마이페이지
        </button>
        <button value={publicpageText} className="homeBtn" onClick={onClickBtn}>
          5. 다른사람의 일정
        </button>
      </div>
      <div className="text">{btnName}</div>
    </HomeTemplete>
  );
};
